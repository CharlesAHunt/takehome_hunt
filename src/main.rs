    ...
    for val in resp_iterator {
        ...
        let pair_stat = match ds.stats.get(pair_key) {
            ...
            Some(stats) => {
                if (latest_price - stats.mean) > stats.std_dev {
                    let alert = models::Alert {
                        ...
                        data: models::AlertMetaData {
                            ...
                            deviation: Option::from(if stats.std_dev == 0.0 {
                                stats.std_dev
                            } else {
                                latest_price / stats.std_dev
                            }),
                ...
                ds.price_history
                    .insert(pair_key.to_owned(), price_history.to_owned());
                let new_mean = statistical::mean(&price_history);
                let new_std_dev =
                    statistical::standard_deviation(&price_history, Option::from(new_mean));
                models::PairStats {
                    std_dev: new_std_dev,
                    mean: new_mean,
                    most_recent_price: latest_price,
